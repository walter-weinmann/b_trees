...
insert_b_tree_order_9_even_odd_test(Config) ->
    Keys = ?config(key_values_from_even_odd, Config),
    _BTree = test_generator:generate_b_tree_from_list(9, Keys),
    ?assertEqual(?config(btree_9, Config), _BTree),
    ?assert(b_trees:height(BTree) =< trunc((math:log((?NUMBER_INSERTS + 1) / 2) / math:log(9 div 2)))).
...
insert_b_tree_order_9_odd_even_test(Config) ->
    Keys = ?config(key_values_from_odd_even, Config),
    _BTree = test_generator:generate_b_tree_from_list(9, Keys),
    ?assertEqual(?config(btree_9, Config), _BTree),
    ?assert(b_trees:height(BTree) =< trunc((math:log((?NUMBER_INSERTS + 1) / 2) / math:log(9 div 2)))).
